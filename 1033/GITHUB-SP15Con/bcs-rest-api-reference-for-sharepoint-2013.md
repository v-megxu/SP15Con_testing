...
|**Operator**||
|:-----|:-----|
|EQ  <br/> |Equals  <br/> *NOTE*  <br/> When you use **EQ** to filter, the filter criteria are passed to the external system where the filtering happens on the server.          |
|GT  <br/> |Greater Than  <br/> *NOTE*  <br/> When you use the **GT** operator, only client-side filtering is executed. <br/> For example:  `web/lists/getByTitle('ListName')/Items?$select=Title&amp;$filter=AverageRating gt 3` returns all titles with an average rating over 3.          |
